                ...
                data.map(item=>{
                    return(
                        <div className="card home-card">
                            <h5 key={item._id}><span style={{padding:"5px"}}><Link to={item.postedBy._id !== state._id ?`/profile/${item.postedBy._id}`:"/profile"}>{item.postedBy.name}</Link></span>{item.postedBy._id===state._id && <i className="material-icons" style={{float:"right", padding:"5px",paddingRight:"5px"}} onClick={()=>deletePost(item._id)}>delete</i>}</h5> 
